// Package oteltraces provides functionality for processing trace data.
package oteltraces
...
const signalType = "oteltraces"

var errUnmarshalFailed = errors.New("failed to unmarshal traces")
...
// OtelTraces is a struct that handles processing of trace data.
...
//
//go:generate mockgen -package oteltraces -source oteltraces.go -destination oteltraces_mock.go
type Client interface {
	Do(req *http.Request) (*http.Response, error)
}

// New creates a new OtelTraces instance.
func New(
	config *config.Config,
	client Client,
	logger log.Logger,
	...
	if cert.TLSEnabled(&config.Traces.TLS) {
		tlsConfig, err := cert.CreateTLSConfig(&config.Traces)
		if err != nil {
...
func (o *OtelTraces) Handler(resp http.ResponseWriter, req *http.Request) {
	...
	result, err := proto.Unmarshal(req, reflect.TypeFor[*tracepb.TracesData]())
	if err != nil {
	...
	traces, ok := result.(*tracepb.TracesData)
	if !ok || traces == nil {
	...
	err = o.dispatch(ctx, o.partition(ctx, traces))
	if err != nil {
		logger.Error(ctx, o.logger, err.Error(), signalTypeLogAttr())
		http.Error(
			resp,
...
func (o *OtelTraces) partition(
	ctx context.Context,
	req *tracepb.TracesData,
) map[string]*tracepb.TracesData {
	ctx, span := o.tracer.Start(
		ctx,
		"oteltraces.partition",
		trace.WithAttributes(
			signalTypeAttr(),
		),
	)
	...
	for _, resouceSpan := range req.GetResourceSpans() {
		logger.Trace(
			ctx,
			o.logger,
			fmt.Sprintf("%+v", resouceSpan.GetResource().GetAttributes()),
			signalTypeLogAttr(),
		)
		...
		if o.config.Tenant.Label != "" {
			for _, attr := range resouceSpan.GetResource().GetAttributes() {
		...
		if tenant == "" {
			if o.config.Tenant.Default == "" {
				logger.Warn(
					ctx,
					o.logger,
					"no tenant found in span attributes and no default tenant configured",
					signalTypeLogAttr(),
				)
			...
			resouceSpan.GetResource().Attributes = append(
				resouceSpan.GetResource().Attributes,
				&common.KeyValue{
					Key: o.config.Tenant.Label,
					Value: &common.AnyValue{
						Value: &common.AnyValue_StringValue{StringValue: tenant},
					},
				},
			)
		}
...
func (o *OtelTraces) dispatch(ctx context.Context, tenantMap map[string]*tracepb.TracesData) error {
	...
	for tenant, traces := range tenantMap {
		...
		go func(tenant string, traces *tracepb.TracesData) {
			...
			resp, err := o.send(ctx, tenant, traces)
			if err != nil {
				o.otelLgtmProxyRecords.Add(ctx,
					int64(len(traces.GetResourceSpans())),
			...
			o.otelLgtmProxyRequests.Add(ctx,
				1,
				metric.WithAttributes(
					tenantAttribute,
					signalTypeAttr(),
			...
			logger.Trace(
				ctx,
				o.logger,
				fmt.Sprintf("%+v", traces.GetResourceSpans()),
				signalTypeLogAttr(),
			)
...
func (o *OtelTraces) send(
	...
	ctx, span := o.tracer.Start(
		...
		trace.WithAttributes(
			...
			attribute.Int("signal.tenant.records", len(traces.GetResourceSpans())),
		),
	)
	defer span.End()
	...
	if err != nil {
		return http.Response{}, fmt.Errorf("failed to marshal traces: %w", err)
	}

	req, err := http.NewRequestWithContext(
		ctx,
		http.MethodPost,
		o.config.Traces.Address,
	...
	request.AddHeaders(tenant, req, o.config, o.config.Traces.Headers)
	...
	o.otelLgtmProxyLatency.Record(ctx,
		...
		metric.WithAttributes(respAttr,
			signalTypeAttr(),
			attribute.String("signal.tenant", tenant),
		),
	)
